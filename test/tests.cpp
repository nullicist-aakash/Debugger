...
namespace {
    ...
    std::int64_t get_section_load_bias(const std::filesystem::path& path, Elf64_Addr file_address) {
    ...
    std::int64_t get_entry_point_offset(const std::filesystem::path& path) {
...
TEST_CASE("Breakpoint on address works", "[breakpoint]") {
    ...
    auto proc = process::launch("targets/hello_sdb", true, channel.get_write_fd());
    channel.close_write();
